...
const convertStyle = (style: any): any => {
  ...
  if (
    rnStyle.backgroundImage &&
    typeof rnStyle.backgroundImage === "string" &&
    rnStyle.backgroundImage.includes("gradient")
  ) {
...
const stylesGlobal: StylesGlobal = {
  ...
  colors: {
    ...
    gradients: {
      ...
      glass:
        "linear-gradient(135deg, rgba(247, 246, 244, 0.95) 0%, rgba(254, 252, 243, 0.8) 100%)",
  ...
  animations: {
    ...
    transitions: {
      ...
      colors:
        "color 200ms cubic-bezier(0, 0, 0.2, 1), background-color 200ms cubic-bezier(0, 0, 0.2, 1)",
    ...
    legacy: {
      ...
      convertStyle: (oldStyleObject: any): any => {
        ...
        Object.keys(newStyle).forEach((key) => {
          if (
            typeof newStyle[key] === "string" &&
            newStyle[key].startsWith("#")
          ) {
            const newColor = stylesGlobal.utils.legacy.getOldColor(
              newStyle[key],
            );
...
const getResponsiveStyle = (styles: {
  ...
  if (isLargeScreen || width >= 768)
    return styles.tablet || styles.large || styles.medium || styles.small;
